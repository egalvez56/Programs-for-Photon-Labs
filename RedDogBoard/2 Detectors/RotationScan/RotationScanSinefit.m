...
% Modified to rotate a waveplate. After the scan, which has points
% before and after the sweet spot, the program fits a Sine 
% curve. It uses fitting function sinefit.  KG 5/22
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc;
clear;
close all;
...
defaultans = {'COM20','COM16','-600','600','10','2','Sinecurve'};%[1 length(dlg_title)+10],[1 5;1 5;1 5;1 30]
...
cest(2)=input(prompt);
prompt = ' Enter estimate ofinitial phase: ';
cest(4)=0;
...
xhi = input(prompt);
countfitlo=floor((xlo-StartVoltage)/Increment+1);
countfithi=floor((xhi-StartVoltage)/Increment);
