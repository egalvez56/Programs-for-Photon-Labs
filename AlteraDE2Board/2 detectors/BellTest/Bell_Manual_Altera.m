...
defaultans = {'COM19','17','1','Bell'};%[1 length(dlg_title)+10],[1 5;1 5;1 5;1 30]
...
numofstates= str2double(userinput{2}); % time interval for each measurement in seconds
...
ax2.XLim=[0 numofstates*timeinterval+1];
...
set(ax2,'XTick',1:1*timeinterval:numofstates*timeinterval)
...
ax3.XLim=[0 numofstates*timeinterval+1];
...
set(ax3,'XTick',1:1*timeinterval:numofstates*timeinterval)
...
while ~isequal(count,loop+1)
    ...
    descr = ['Tomography: ','State # ',num2str(stateindexi),'/',num2str(numofstates),', measurement #',num2str(count),'/',num2str(numofmeasurements)];
