%% RotationAcanQuadraticfit 
%  
% Program to find the right orientation of the quartz plate for 
% making the HH+VV entangled state.
% To work the polarization projections have to be set to D and A
% The program rotates the quartz plate using PacificLaser stepper motor
% to fins the minimum in DA correlations. 
% It records a scan of the rotation of the quartz plate
% with Alice and Bob at D and A, respectively. HWP for the pump
% is set for equal amounts of HH and VV through a different program
% "EntanglementTune". After the scan, which has points
% before and after the sweet spot, the program fits a nonlinear 
% parabola. It uses fitting function quadraticmin.  KG 5/22
%
% The structure of the input/display components of this program 
% were by Bhzad Khajavi in July 2017.             
...
defaultans = {'COM24','COM16','0','70','5','2','Quartzplate'};%[1 length(dlg_title)+10],[1 5;1 5;1 5;1 30]
...
StartAngleNods = str2double(userinput{3});
EndAngleNods = str2double(userinput{4});
DeltaA=EndAngleNods-StartAngleNods;
...
%         flushinput(CD48serial)
        ...
        VtoCD48 = StartAngleNods + (count-1)*Increment;
...
%% Fit
%
disp('Fitting the minimum')
prompt = ' Enter estimated mimimum position: ';
cest(2)=input(prompt);
prompt = ' Enter mimimum y value: ';
cest(3)=input(prompt);
cest(4)=0;
prompt = ' For a pair of values enter x: ';
xval=input(prompt);
prompt = ' For a pair of values enter y: ';
yval=input(prompt);
cest(1)=(yval-cest(3))/(xval-cest(2))^2;
prompt = ' Enter low x value of data: ';
xlo = input(prompt);
prompt = ' Enter high x value:';
xhi = input(prompt);
countfitlo=floor((xlo-StartAngleNods)/Increment)+1;
countfithi=floor((xhi-StartAngleNods)/Increment)+1;
xinfit=voltage(countfitlo:countfithi)';
yinfit=resultsmatrix(countfitlo:countfithi,3);
uncy=sqrt(yinfit);
cfit=lsqcurvefit(@quadraticmin,cest,xinfit,yinfit);
yfit=quadraticmin(cfit,xinfit);
figure(2);
errorbar(xinfit,yinfit,uncy,'bo','Markersize',3);
hold on
plot(xinfit,yfit,'r-');
cmin=num2str(cfit(2));
title([' Minimum fit: ',cmin]);
hold off
...
disp(' ');
disp('y=c(1)*(x-c(2)).^2+c(4)*(x-c(2)).^3+c(3) ');
...
% quadratic function
%
function [ y ]=quadraticmin (c,x)
y=c(1)*(x-c(2)).^2+c(4)*(x-c(2)).^3+c(3);
end