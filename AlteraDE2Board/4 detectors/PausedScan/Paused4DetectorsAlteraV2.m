...
% Paused scan with 4 detectors 
% Takes data on all 4 inputs recording all doubles, pausing on each point
% Original Version Behzad Khajavi and Baibhav Sharma 5/2018
% Revision with capabilities for short and long times Kiko Galvez 4/2/2022
% Modified for clearing input buffers 5/29/22
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
prompt = {'Enter the COM# of the Counter:','number of intervals','time per interval','Name of file = ','Coincidence Time (ns)'};
dlg_title = 'Four-detector Dialog';
defaultans = {'COM7','5','1','Paused4','8'};%[1 length(dlg_title)+10],[1 5;1 5;1 5;1 30]
...
Header1={'A','B','A`','B`','AB','BA`','AB`','A`B`','Acc-AB','Acc-BA`','Acc-ABB`','Acc-A`B`'};
...
warning('off','MATLAB:xlswrite:AddSheet');
% to suppress the warning when the sheet name is not in excel file.
Header2={'A','B','A`','B`','AB','BA`','AB`','A`B`','Acc-AB','Acc-BA`','Acc-AB`','Acc-A`B`'};
xl2range1='A1';
...
%            pause(1);
        ...
        xlrange2=strcat('A',countt);
        accidentalsAB=resultsmatrix(count,1)*resultsmatrix(count,2)*deltat/timeinterval;
        accidentalsBAp=resultsmatrix(count,2)*resultsmatrix(count,3)*deltat/timeinterval;
        accidentalsABp=resultsmatrix(count,1)*resultsmatrix(count,4)*deltat/timeinterval;
        accidentalsApBp=resultsmatrix(count,3)*resultsmatrix(count,4)*deltat/timeinterval;
        ...
        descripion = strcat('State # ',' ',countstr,':');
        defaultans = {[descripion,' Do not enter anything here.']};
        if count==numofmeasurements
...
save(strcat(nams,'.txt'),'resultsmatrix','-ascii')
